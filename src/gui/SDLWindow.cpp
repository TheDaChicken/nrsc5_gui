	...
	const int height)
{
	...
	auto new_window = std::unique_ptr<SDL_Window, decltype(&SDL_DestroyWindow)>(SDL_CreateWindow(
			name.c_str(),
	...
	if (!SDL_ClaimWindowForGPUDevice(gpu_device->GetDevice(), new_window.get()))
	{
	...
	init_info.Device = gpu_device->GetDevice();
	init_info.ColorTargetFormat = SDL_GetGPUSwapchainTextureFormat(gpu_device->GetDevice(), new_window.get());
	init_info.MSAASamples = SDL_GPU_SAMPLECOUNT_1; // Only used in multi-viewports mode.
	ImGui_ImplSDLGPU3_Init(&init_info);

	m_window = std::move(new_window);
	gpu_context_ = gpu_device;
	return true;
}
...
void SDLWindow::DestroyWindow()
{
	...
	SDL_ReleaseWindowFromGPUDevice(gpu_context_->GetDevice(), m_window.get());
	m_window.reset();
}
...
void SDLWindow::SubmitFrame() const
{
	// Rendering
	ImGui::Render();
	...
	SDL_GPUCommandBuffer *command_buffer = SDL_AcquireGPUCommandBuffer(gpu_context_->GetDevice());
