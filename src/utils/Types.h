//
// Created by TheDaChicken on 5/5/2024.
//

#ifndef NRSC5_GUI_SRC_LIB_UTILS_TYPES_H_
#define NRSC5_GUI_SRC_LIB_UTILS_TYPES_H_

#include <vector>
#include <cstdint>
#include <complex>

using cfloat_t = std::complex<float>;
using cuint8_t = std::complex<uint8_t>;
using cint16_t = std::complex<int16_t>;
using cint32_t = std::complex<int32_t>;
using cint64_t = std::complex<int64_t>;

using vector_int16_t = std::vector<int16_t>;
using vector_uint8_t = std::vector<uint8_t>;
using vector_complex_t = std::vector<cfloat_t>;
using vector_cint16_t = std::vector<cint16_t>;
using vector_float_t = std::vector<float>;


inline void cint16_dot_product(cint16_t *output, const cint16_t *in_a, const cint16_t *in_b, const size_t len)
{
	int32_t acc_real = 0;
	int32_t acc_imag = 0;

	for (int i = 0; i < len; ++i)
	{
		const int32_t ar = in_a[i].real();
		const int32_t ai = in_a[i].imag();
		const int32_t br = in_b[i].real();
		const int32_t bi = in_b[i].imag();

		acc_real += ar * br - ai * bi;
		acc_imag += ar * bi + ai * br;
	}

	// Scale back to Q15
	acc_real >>= 15;
	acc_imag >>= 15;

	output[0] = cint16_t(static_cast<int16_t>(acc_real), static_cast<int16_t>(acc_imag));
}
#endif //NRSC5_GUI_SRC_LIB_UTILS_TYPES_H_
