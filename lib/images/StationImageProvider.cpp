...
void StationImageProvider::AddProvider(const std::shared_ptr<IImageProvider>& provider, int priority) const
{
  provider_manager_->AddProvider(provider, priority);
}

ImageData StationImageProvider::FetchStationImage(const RadioChannel &channel) const
{
...
ImageData StationImageProvider::FetchPrimaryImage(const RadioChannel &channel,
