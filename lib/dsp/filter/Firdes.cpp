...
std::vector<float> Firdes::window(Window::win_type type, int ntaps, double param)
{
    return Window::build(type, ntaps, param);
}
...
std::vector<float> Firdes::low_pass(double gain,
...
std::vector<float> Firdes::high_pass_2(double gain,
                                  double sampling_freq,
                                  double cutoff_freq,      // Hz center of transition band
                                  double transition_width, // Hz width of transition band
                                  double attenuation_dB,   // attenuation dB
...
std::vector<float> Firdes::high_pass(double gain,
                                double sampling_freq,
                                double cutoff_freq,      // Hz center of transition band
                                double transition_width, // Hz width of transition band
                                Window::win_type window_type,
...
std::vector<float> Firdes::band_pass_2(double gain,
                                  double sampling_freq,
...
std::vector<float> Firdes::band_pass(double gain,
                                double sampling_freq,
                                double low_cutoff_freq,  // Hz center of transition band
                                double high_cutoff_freq, // Hz center of transition band
                                double transition_width, // Hz width of transition band
                            ...
                            double param) // used with Kaiser, Exp., Gaussian, Tukey
{
    ...
    std::vector<std::complex< float >> taps(ntaps);
    ...
    for (unsigned int i = 0; i < lptaps.size(); i++) {
        *optr++ = std::complex< float >(*iptr * cos(phase), *iptr * sin(phase));
        iptr++, phase += freq;
    }
...
std::vector<float> Firdes::band_reject(double gain,
                                  double sampling_freq,
                                  double low_cutoff_freq,  // Hz center of transition band
                                  double high_cutoff_freq, // Hz center of transition band
                                  double transition_width, // Hz width of transition band
...
std::vector<float>
Firdes::hilbert(unsigned int ntaps, Window::win_type windowtype, double param)
{
    if (!(ntaps & 1))
        throw std::out_of_range("Hilbert:  Must have odd number of taps");

    std::vector<float> taps(ntaps);
    std::vector<float> w = window(windowtype, ntaps, param);
...
std::vector<float> Firdes::gaussian(double gain, double spb, double bt, int ntaps)
{
    std::vector<float> taps(ntaps);
    double scale = 0;
    double dt = 1.0 / spb;
...
std::vector<float> Firdes::root_raised_cosine(
...
int Firdes::compute_ntaps(double sampling_freq,
                          ...
                          double param)
{
    double a = Window::max_attenuation(window_type, param);
